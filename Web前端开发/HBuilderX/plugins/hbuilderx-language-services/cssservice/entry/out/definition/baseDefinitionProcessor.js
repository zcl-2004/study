...
exports.BaseDefinitionProcessor = void 0;
// 转到定义功能不需要区分语言, 共用同一套逻辑, 以后可能有不同的实现, 所以此处也创建基类
...
class BaseDefinitionProcessor {
    // TODO逻辑太乱了, 后续整理
    // 类型转换, Location不带当前可转到定义词汇的位置信息, 故做此转换
    ...
    getDefinitionDataFromAtPath(workspaceFolder, document, position, astNode, definitionData) {
        ...
        if (currentText.startsWith('@') && currentAstNode.parent.nodeType === util_1.NodeType.URILiteral) {
            ...
            locationList.unshift({
                uri: out_1.hx.toNormalizedUri(absolutePath),
    ...
    // 提供接口让外部获取基本转到定义信息(html中的css自身的转到定义)
...
exports.BaseDefinitionProcessor = BaseDefinitionProcessor;
//# sourceMappingURL=baseDefinitionProcessor.js.map