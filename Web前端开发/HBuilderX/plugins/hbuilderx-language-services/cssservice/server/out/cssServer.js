...
function startServer(connection, runtime) {
    ...
    connection.onCompletion((textDocumentPosition, token) => {
        return (0, runner_1.runSafeAsync)(async () => {
            ...
            if (document) {
                ...
                completionList = await completionClass.doExtraCompletion(workspaceFolder, document, position, astNode, connection, scopedSettingsSupport, completionList);
    ...
    connection.onDocumentSymbol((documentSymbolParams, token) => {
        return (0, runner_1.runSafeAsync)(async () => {
            ...
            if (document) {
                ...
                let hxSymbolInformationList = ((0, entry_1.getExtraServer)(document).doDocumentSymbol(workspaceFolder, symbolInformationList));
    ...
    connection.onDefinition((documentDefinitionParams, token) => {
        return (0, runner_1.runSafeAsync)(async () => {
            ...
            if (document) {
                ...
                // 将新增的功能添加到原先的结果(多为ID跨文件跳转)
                let definitionDataList = await ((0, entry_1.getExtraServer)(document).doDefinition(workspace, document, documentDefinitionParams.position, stylesheet, location));
    ...
    connection.onDocumentFormatting((formatParams, token) => {
        return (0, runner_1.runSafeAsync)(async () => {
            ...
            if (document) {
                await dataProvidersReady;
                const definitionDataList = await ((0, entry_1.getExtraServer)(document).doFormatting(document, formatParams.options, connection, scopedSettingsSupport));
                return definitionDataList;
            }
            return [];
