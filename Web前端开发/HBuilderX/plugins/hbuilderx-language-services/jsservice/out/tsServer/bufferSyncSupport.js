...
class BufferSynchronizer {
    constructor(client, onCaseInsenitiveFileSystem) {
        this.client = client;
        this._pending = new resourceMap_1.ResourceMap(undefined, {
    ...
    flush() {
        ...
        if (this._pending.size > 0) {
            ...
            console.log(`buffer sync supports: ${JSON.stringify(changedFiles)}`);
            this._pending.clear();
        }
    }
...
class PendingDiagnostics extends resourceMap_1.ResourceMap {
    getOrderedFileSet() {
        ...
        const map = new resourceMap_1.ResourceMap(undefined, this.config);
...
class GetErrRequest {
    constructor(client, files, onDone) {
        ...
        const allFiles = (0, arrays_1.coalesce)(Array.from(files.entries).map(entry => client.normalizedPath(entry.resource)));
...
class BufferSyncSupport extends dispose_1.Disposable {
    ...
    listen() {
        ...
        //TODO 这里如果要在tsserver启动的时候立马就初始化项目，可以在这里构造假文档实现tsserver的初始化
    }
    handles(resource) {
