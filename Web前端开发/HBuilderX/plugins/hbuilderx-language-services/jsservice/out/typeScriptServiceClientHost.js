...
class TypeScriptServiceClientHost extends dispose_1.Disposable {
    constructor(descriptions, workspaceState, onCaseInsenitiveFileSystem, services, onCompletionAccepted) {
        ...
        this.client.onDiagnosticsReceived(({ kind, resource, diagnostics }) => {
            //TODO 暂时屏蔽掉实时校验，workspace.openTextDocument会打开文档，导致文件来回切换
            this.diagnosticsReceived(kind, resource, diagnostics);
        }, null, this._disposables);
        //TODO 这个地方会调用openTextDocument打开tsconfig.json文件，暂时屏蔽
    ...
    async findLanguage(resource) {
        try {
            ...
            let resourcePath = this.client.toPath(resource);
            const doc = vscode.workspace.textDocuments.find((openedDoc) => {
                let openedDocPath = this.client.toPath(openedDoc.uri);
                return openedDocPath === resourcePath;
            });
