...
class MyCompletionItem extends vscode.CompletionItem {
    constructor(position, document, tsEntry, completionContext, metadata) {
        ...
        this.position = position;
        this.useCodeSnippet = (this.kind === vscode.CompletionItemKind.Function || this.kind === vscode.CompletionItemKind.Method);
...
class ApplyCompletionCodeActionCommand {
    ...
    async execute(_file, codeActions) {
        ...
        return (0, codeAction_1.applyCodeAction)(this.client, action, cancellation_1.nulToken);
    }
}
...
class TypeScriptCompletionItemProvider {
    ...
    async resolveCompletionItem(item, token) {
        ...
        const args = {
            ...
            entryNames: [
                //TypeScript 4.3+之后协议有更新
                item.tsEntry.source || item.tsEntry.data ? {
                    name: item.tsEntry.name,
                    source: item.tsEntry.source,
                    data: item.tsEntry.data
                } : item.tsEntry.name
            ]
        };
        ...
        let sourceFile = ts.createSourceFile(item.document.uri.fsPath, item.document.getText(), ts.ScriptTarget.ESNext, true);
        let data = utils_1.hx.computeJSResolveData(item.label, item.detail, item.kind, item.document.offsetAt(item.position), item.textEdit, sourceFile, true);
        let prefix = '';
        if (typeof item.insertText === 'string') {
            let index = item.insertText.indexOf(item.label);
            if (index !== -1)
                prefix = item.insertText.substring(0, item.insertText.length - item.label.length);
        }
        if (data.insertText)
            item.insertText = new vscode.SnippetString(`${prefix}${data.insertText}`);
        if (data.command)
        ...
        if (item.label === 'async' || item.label === 'await') {
            // 判断插入的文本是不是和label一样
            // 如果不是, 则修改为代码块插入, 把光标放在空格前面
            if (item.tsEntry && item.tsEntry.kindModifiers === 'Snippet') {
                item.insertText = new vscode.SnippetString(item.tsEntry.insertText);
            }
        }
        return item;
    }
