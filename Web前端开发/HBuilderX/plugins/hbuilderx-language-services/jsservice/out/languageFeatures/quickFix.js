...
class ApplyFixAllCodeAction {
    ...
    async execute(file, tsAction) {
        ...
        */
        ...
        await (0, codeAction_1.applyCodeActionCommands)(this.client, response.body.commands, cancellation_1.nulToken);
    }
}
...
class VsCodeCodeAction extends vscode.CodeAction {
    constructor(tsAction, title, kind, isFixAll) {
        super(title, kind);
        this.tsAction = tsAction;
        this.isFixAll = isFixAll;
    }
}
class CodeActionSet {
    constructor() {
...
class TypeScriptQuickFixProvider {
    ...
    addFixAllForTsCodeAction(results, document, file, diagnostic, tsAction) {
        ...
        const action = new VsCodeCodeAction(tsAction, tsAction.fixAllDescription || localize('fixAllInFileLabel', '{0} (Fix all in file)', tsAction.description), vscode.CodeActionKind.QuickFix, true);
