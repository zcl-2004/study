...
const css_parser2_1 = require("css-parser2");
...
function updateEmmetExtensionsPath(forceRefresh = false) {
    ...
    // const emmetConfig = getPackageJsonConfiguration();
    // let extensionsPath = emmetConfig['extensionsPath'];
    let extensionsPath = [];
    if (!extensionsPath) {
        extensionsPath = [];
    }
    if (forceRefresh || _currentExtensionsPath !== extensionsPath) {
        ...
        const rootPaths = ((_a = vscode.workspace.workspaceFolders) === null || _a === void 0 ? void 0 : _a.length) ? vscode.workspace.workspaceFolders.map(f => f.uri) : undefined;
...
function validate(allowStylesheet = true) {
    ...
    if (!editor) {
        console.error('Emmet Error: No editor is active. (util.ts.ts::validate)');
        return false;
    }
...
function getMappingForIncludedLanguages() {
    ...
    // const includeLanguagesConfig = emmetConfig.get<Record<string, string>>('includeLanguages');
    const includeLanguagesConfig = undefined;
    const includeLanguages = Object.assign({}, MAPPED_MODES, includeLanguagesConfig !== null && includeLanguagesConfig !== void 0 ? includeLanguagesConfig : {});
...
function parsePartialStylesheet(document, position) {
    ...
    try {
        ...
        return (0, css_parser2_1.default)(buffer);
    }
    catch (e) {
        return;
    }
...
function getHtmlFlatNode(documentText, root, offset, includeNodeBoundary) {
    ...
    if (currentNode.name === 'script' && currentNode.children.length === 0) {
        ...
        if (scriptNodeBody) {
            currentNode = (_a = getHtmlFlatNode(scriptNodeBody, currentNode, offset, includeNodeBoundary)) !== null && _a !== void 0 ? _a : currentNode;
        }
    }
    else if (currentNode.type === 'cdata') {
        const cdataBody = setupCdataNodeSubtree(documentText, currentNode);
        currentNode = (_b = getHtmlFlatNode(cdataBody, currentNode, offset, includeNodeBoundary)) !== null && _b !== void 0 ? _b : currentNode;
    }
    return currentNode;
}
...
function getPackageJsonConfiguration() {
    var _a, _b;
    let pkgInfo = require('../package.json');
    let emmetConfig = {};
    let emmetConfigProperties = (_b = (_a = pkgInfo === null || pkgInfo === void 0 ? void 0 : pkgInfo.contributes) === null || _a === void 0 ? void 0 : _a.configuration) === null || _b === void 0 ? void 0 : _b.properties;
    if (emmetConfigProperties) {
        for (let property in emmetConfigProperties) {
            emmetConfig[property.substring(6)] = emmetConfigProperties[property].default;
        }
    }
    return emmetConfig;
}
...
function getEmmetConfiguration(syntax) {
    ...
    if (syntax === 'jsx' || syntax === 'xml' || syntax === 'xsl') {
        ...
        if (typeof syntaxProfiles[syntax] === 'object'
            ...
            syntaxProfiles[syntax] = Object.assign(Object.assign({}, syntaxProfiles[syntax]), { selfClosingStyle: syntax === 'jsx' ? 'xhtml' : 'xml' });
        }
    }
    return {
        preferences,
...
function getCssPropertyFromDocument(editor, position) {
    ...
    if (htmlNode
        ...
        const innerRootNode = (0, css_parser2_1.default)(buffer);
