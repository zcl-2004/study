...
function wrapWithAbbreviation(args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        function makeChanges(inputAbbreviation, previewChanges) {
            return __awaiter(this, void 0, void 0, function* () {
        ...
        const inputAbbreviation = (args && args['abbreviation'])
            ? args['abbreviation']
            : yield vscode.window.showInputBox({ prompt, validateInput: inputChanged });
        const changesWereMade = yield makeChanges(inputAbbreviation, false);
        if (!changesWereMade) {
...
function expandEmmetAbbreviation(args) {
    ...
    let syntax = getSyntaxFromArgs(args);
    if (!syntax) {
        return fallbackTab();
    }
    const editor = vscode.window.activeTextEditor;
    // When tabbed on a non empty selection, do not treat it as an emmet abbreviation, and fallback to tab instead
    // if (emmetConfig['triggerExpansionOnTab'] === true && editor.selections.find(x => !x.isEmpty)) {
    ...
    function getRootNode() {
        ...
        const emmetConfig = (0, util_1.getPackageJsonConfiguration)();
        const usePartialParsing = emmetConfig['optimizeStylesheetParsing'] === true;
    ...
    selectionsInReverseOrder.forEach(selection => {
        ...
        if (args['language'] === 'vue') {
            const vueDocument = editor.document;
            const positionOffset = vueDocument.offsetAt(position);
            const emmetRootNode = (0, parseDocument_1.getRootNode)(vueDocument, true);
            const foundNode = (0, util_1.getHtmlFlatNode)(vueDocument.getText(), emmetRootNode, positionOffset, false);
            if ((foundNode === null || foundNode === void 0 ? void 0 : foundNode.name) === 'style') {
                syntax = 'css';
            }
        }
    ...
    return expandAbbreviationInRange(editor, abbreviationList, allAbbreviationsSame).then(success => {
        return success ? Promise.resolve(true) : fallbackTab();
    });
}
exports.expandEmmetAbbreviation = expandEmmetAbbreviation;
function fallbackTab() {
    // if (emmetConfig['triggerExpansionOnTab'] === true) {
...
function expandAbbr(input) {
    ...
    try {
        ...
        console.error('[PluginHost] EMMEt: Failed to expand abbreviation');
...
function getSyntaxFromArgs(args) {
    ...
    let syntax = (0, util_1.getEmmetMode)((_a = mappedModes[language]) !== null && _a !== void 0 ? _a : language, mappedModes, excludedLanguages);
    if (!syntax) {
        syntax = (0, util_1.getEmmetMode)((_b = mappedModes[parentMode]) !== null && _b !== void 0 ? _b : parentMode, mappedModes, excludedLanguages);
    }
    return syntax;
}
