...
function findExportThisExpressionAtOffset(source, position) {
    if (!source) {
        return undefined;
    }
    let exportAssignNode = source.statements.find(ts.isExportAssignment);
    if (exportAssignNode) {
        let token = getTokenAtPosition(source, position - 1);
        if (token.getStart() <= position && position <= token.getEnd()) {
            let nodepath = (0, nodes_1.findNodePathByOffset)(source, position);
            for (let i = nodepath.length - 1; i >= 0; --i) {
                ...
                if (node.getStart() <= position && position <= node.getEnd()) {
                    if (node.kind == ts.SyntaxKind.CallExpression ||
                        ...
                        if (expr) {
                            ...
                            while (parentNode && parentNode.kind !== ts.SyntaxKind.ExportAssignment &&
                                parentNode.kind !== ts.SyntaxKind.FunctionDeclaration) {
                                parentNode = parentNode.parent;
                            }
                            if (!parentNode || parentNode.kind != ts.SyntaxKind.FunctionDeclaration) {
                                return node;
                            }
                        }
                    }
                ...
                //   return { expresion: node, directly: false };
                // }
            }
        }
...
exports.findExportThisExpressionAtOffset = findExportThisExpressionAtOffset;
function findExportDefaultObjectExpressionAtKeyOffset(source, position) {
    if (!source) {
        return undefined;
    }
    let exportAssignNode = source.statements.find(ts.isExportAssignment);
    if (exportAssignNode) {
        let expr = exportAssignNode.expression;
        if (expr && ts.isObjectLiteralExpression(expr) && expr.pos <= position && expr.end >= position) {
            let properties = expr.properties;
            let child = properties.find(item => {
                if (item.pos > position || item.end < position) {
                    return undefined;
                }
                if (item.name && item.name.pos <= position && item.name.end >= position) {
                    return expr;
                }
                return undefined;
            });
            let lastChild = properties.length > 0 ? properties[properties.length - 1] : undefined;
            if (child || lastChild == undefined || lastChild.end < position) {
                return expr;
            }
        }
    }
...
exports.findExportDefaultObjectExpressionAtKeyOffset = findExportDefaultObjectExpressionAtKeyOffset;
function inExportDefaultObjectExpression(source, position) {
    return findExportDefaultObjectExpressionAtKeyOffset(source, position) !== undefined;
}
exports.inExportDefaultObjectExpression = inExportDefaultObjectExpression;
function isStringLiteral(sourceFile, position) {
    let token = getTokenAtPosition(sourceFile, position);
    if ((token === null || token === void 0 ? void 0 : token.kind) === ts.SyntaxKind.StringLiteral) {
        let tokenText = token.getText();
        return {
            isInString: true,
            flag: tokenText.startsWith(`'`) ? `'` : `"`
        };
    }
    return { isInString: false };
}
exports.isStringLiteral = isStringLiteral;
function isObjectLiteralKey(source, position) {
    let token = getTokenAtPosition(source, position);
    while (true) {
        if (!token) {
            break;
        }
        if (ts.isSourceFile(token)) {
            break;
        }
        if (ts.isObjectLiteralExpression(token)) {
            break;
        }
        token = token.parent;
    }
    if (token && ts.isObjectLiteralExpression(token)) {
...
exports.isObjectLiteralKey = isObjectLiteralKey;
function isImportElement(source, position) {
    let token = getTokenAtPosition(source, position);
    if (token.parent &&
        (token.parent.kind == ts.SyntaxKind.NamedImports
            || token.parent.kind == ts.SyntaxKind.ImportSpecifier || isJSX(token))) {
        return true;
    }
    return false;
}
exports.isImportElement = isImportElement;
// 是否是jsxdom
function isJSX(token) {
    if (token.kind === ts.SyntaxKind.JsxText || token.kind === ts.SyntaxKind.Identifier) {
        return token.parent.kind === ts.SyntaxKind.JsxSelfClosingElement || token.parent.kind === ts.SyntaxKind.JsxElement;
    }
    return false;
}
//# sourceMappingURL=type-resolve.js.map