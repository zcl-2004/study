...
class StylusIndexProcessor {
    support(doc, _ws) {
        if (doc.languageId === 'stylus' || doc.uri.endsWith('.styl')) {
            return true;
        }
        return false;
    }
    doIndex(doc, _ws) {
        const value = this.createIndexData(doc);
        if (value) {
            return value;
        }
        return {
    ...
    createIndexData(document) {
        ...
        const filterAst = (0, parser_1.flattenAndFilterAst)(ast);
        if (!filterAst)
            return;
        const rawSymbols = (0, utils_1.compact)(filterAst.filter((item) => item && ['selector', 'import'].indexOf(item.nodeName) !== -1));
