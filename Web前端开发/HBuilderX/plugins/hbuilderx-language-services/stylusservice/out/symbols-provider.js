...
function processRawSymbols(rawSymbols, text) {
    return rawSymbols.map(symNode => {
        if ((0, parser_1.isVariableNode)(symNode)) {
            return _variableSymbol(symNode, text);
        }
        if ((0, parser_1.isFunctionNode)(symNode)) {
            return _functionSymbol(symNode, text);
        }
        if ((0, parser_1.isSelectorNode)(symNode)) {
            return _selectorSymbol(symNode, text);
        }
        if ((0, parser_1.isSelectorCallNode)(symNode)) {
            return _selectorCallSymbol(symNode, text);
        }
        if ((0, parser_1.isAtRuleNode)(symNode)) {
...
class StylusDocumentSimbolsProvider {
    provideDocumentSymbols(document, token) {
        ...
        return uniq((0, utils_1.compact)(processRawSymbols(rawSymbols, text.split('\n'))));
    }
}
