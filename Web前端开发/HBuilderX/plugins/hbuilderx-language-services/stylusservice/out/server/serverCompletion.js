...
const typeConversion_1 = require("../../../utils/client/out/typeConversion");
/**
...
class StylusServerCompletion {
    provideCompletionItems(document, position) {
        let typeConversion = new typeConversion_1.TypeConversion();
        const start = vscode_languageserver_1.Position.create(position.line, 0);
        const range = new vscode_1.Range(typeConversion.getServerToClient().PositionConversion(start), typeConversion.getServerToClient().PositionConversion(position));
        ...
        const completions = [].concat(symbols, atRules, properties, values, css_built_in_1.default, config.get('useBuiltinFunctions', true) ? built_in_1.default : []);
        return typeConversion.getClientToServer().CompletionListConversion({
            isIncomplete: false,
            items: completions,
        });
    }
