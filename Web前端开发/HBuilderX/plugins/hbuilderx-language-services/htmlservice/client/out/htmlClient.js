...
function startClient(context, newLanguageClient, runtime) {
    ...
    let clientOptions = {
        ...
        synchronize: {
            ...
            fileEvents: vscode_1.workspace.createFileSystemWatcher('**/package.json')
        },
        ...
        outputChannel: new client_1.LogRedirectOutputChannel(localize('htmlserver.name', 'HTML Language Server')),
        middleware: {
    ...
    vscode_1.languages.setLanguageConfiguration('html_es6', {
        indentationRules: {
            increaseIndentPattern: /<(?!\?|(?:area|base|br|col|frame|hr|html|img|input|link|meta|param)\b|[^>]*\/>)([-_\.A-Za-z0-9]+)(?=\s|>)\b[^>]*>(?!.*<\/\1>)|<!--(?!.*-->)|\{[^}"']*$/,
            decreaseIndentPattern: /^\s*(<\/(?!html)[-_\.A-Za-z0-9]+\b[^>]*>|-->|\})/
        },
    ...
    vscode_1.languages.setLanguageConfiguration('vue', {
        indentationRules: {
            increaseIndentPattern: /<(?!\?|(?:area|base|br|col|frame|hr|html|img|input|link|meta|param)\b|[^>]*\/>)([-_\.A-Za-z0-9]+)(?=\s|>)\b[^>]*>(?!.*<\/\1>)|<!--(?!.*-->)|\{[^}"']*$/,
            decreaseIndentPattern: /^\s*(<\/(?!html)[-_\.A-Za-z0-9]+\b[^>]*>|-->|\})/
        },
    ...
    const promptForTypeOnRename = vscode_1.extensions.getExtension('formulahendry.auto-rename-tag') !== undefined &&
        ...
        !vscode_1.workspace.getConfiguration('editor', { languageId: 'html' }).get('renameOnType');
    if (promptForTypeOnRename) {
        const activeEditorListener = vscode_1.window.onDidChangeActiveTextEditor(async (e) => {
            if (e && documentSelector.indexOf(e.document.languageId) !== -1) {
                ...
                const res = await vscode_1.window.showInformationMessage(localize('renameOnTypeQuestion', 'VS Code now has built-in support for auto-renaming tags. Do you want to enable it?'), configure);
                if (res === configure) {
                    vscode_1.commands.executeCommand('workbench.action.openSettings', SettingIds.renameOnType);
                }
            }
        });
